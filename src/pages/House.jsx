...
export default function House() {
  ...
  useEffect(() => {
    ...
    if (timeData) {
      const now = Date.now();
      const elapsed = Math.floor((now - timeData.startTimestamp) / 250); // tiap 250ms = 1 menit game
    ...
    // lanjutkan waktu berjalan di dalam house
  ...
  const formatTime = (h, m) => `${String(h).padStart(2, "0")}:${String(m).padStart(2, "0")}`;
    ...
    <div className="viewport">
      <div className="map" style={{ backgroundImage: `url(${homeMap})` }}></div>
      ...
      <button
        ...
        onClick={() => {
          // Simpan perubahan status, uang, dan inventory dari House
